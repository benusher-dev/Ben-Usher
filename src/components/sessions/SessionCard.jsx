    ...
    <Card className="p-4" onClick={onClick}>
      <div className="flex items-start justify-between">
        ...
        <div className="text-right flex-shrink-0 ml-4">
          {session.durationMinutes != null && (
            <p className="text-sm font-medium text-indigo-600 dark:text-indigo-400">{formatDuration(session.durationMinutes)}</p>
          )}
          {vol > 0 && (
      ...
      <div className="mt-2 flex flex-wrap gap-1">
        {session.exercises.slice(0, 4).map((ex, i) => (
          <span key={i} className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-lg px-2 py-0.5">
            {ex.name}
          </span>
        ))}
        {session.exercises.length > 4 && (
          <span className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-400 dark:text-gray-500 rounded-lg px-2 py-0.5">
            +{session.exercises.length - 4}
          </span>
        )}
      </div>
